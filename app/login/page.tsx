...
import { Bookmark, Star, Clock, Tag, Zap, Shield, Search } from 'lucide-react'
    ...
    <div className="min-h-screen bg-slate-950 flex overflow-hidden">
      ...
      <div className="hidden lg:flex flex-col flex-1 relative p-14 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          <div>
            <p className="text-[10px] text-slate-700 uppercase tracking-widest font-semibold mb-3">Built with</p>
            <div className="flex flex-wrap gap-2">
              {[
                { name: 'Next.js', bg: 'bg-white/5', text: 'text-slate-300', border: 'border-white/8' },
                { name: 'Tailwind CSS', bg: 'bg-cyan-500/8', text: 'text-cyan-400', border: 'border-cyan-500/15' },
                { name: 'Supabase', bg: 'bg-emerald-500/8', text: 'text-emerald-400', border: 'border-emerald-500/15' },
                { name: 'TypeScript', bg: 'bg-blue-500/8', text: 'text-blue-400', border: 'border-blue-500/15' },
                { name: 'Google OAuth', bg: 'bg-white/5', text: 'text-slate-400', border: 'border-white/8' },
              ].map(({ name, bg, text, border }) => (
                <span key={name} className={`text-[11px] font-medium px-2.5 py-1 rounded-lg border ${bg} ${text} ${border}`}>
                  {name}
                </span>
              ))}
            </div>
      ...
      {/* Vertical divider */}
      <div className="hidden lg:flex flex-col items-center py-14 shrink-0">
        <div className="w-px flex-1 bg-linear-to-b from-transparent to-white/10" />
        <div className="relative my-4">
          <div className="absolute inset-0 bg-blue-500/30 rounded-full blur-md scale-150" />
          <div className="relative w-7 h-7 rounded-full border border-white/10 bg-slate-900 flex items-center justify-center">
            <div className="w-2 h-2 rounded-full bg-blue-500/70" />
          </div>
        </div>
        <div className="w-px flex-1 bg-linear-to-b from-white/10 to-transparent" />
      </div>

      {/* ── Right: login panel ─────────────────────────────────────── */}
      <div className="flex-1 lg:flex-none lg:w-150 flex flex-col items-center justify-center p-8 relative">
        {/* Background — visible on all sizes */}
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="animate-blob animation-delay-0 absolute -top-32 -left-32 w-96 h-96 bg-blue-600/15 rounded-full blur-3xl" />
          <div className="animate-blob animation-delay-2000 absolute -bottom-32 -right-32 w-80 h-80 bg-violet-600/10 rounded-full blur-3xl" />
        </div>
        ...
        <div className="relative w-full max-w-sm">
          ...
          <div className="flex flex-col items-center mb-10">
            <div className="relative mb-5">
              <div className="absolute inset-0 bg-blue-500 rounded-2xl blur-2xl opacity-40 scale-125" />
            ...
            <p className="text-slate-500 text-sm mt-2 text-center leading-relaxed">
              Save, tag, and rediscover the web.
            </p>
          </div>
          ...
          <div className="relative">
            ...
            <div className="absolute -inset-px rounded-2xl bg-linear-to-br from-blue-500/40 via-indigo-500/10 to-violet-500/30 pointer-events-none" />

            <div className="relative bg-slate-900/70 backdrop-blur-2xl rounded-2xl p-7 shadow-2xl">
              {/* What you get */}
              <div className="grid grid-cols-3 gap-3 mb-7">
                {[
                  { Icon: Zap,    label: 'Instant save',  color: 'text-amber-400',  bg: 'bg-amber-500/10'  },
                  { Icon: Search, label: 'Fast search',   color: 'text-blue-400',   bg: 'bg-blue-500/10'   },
                  { Icon: Shield, label: 'Private',       color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
                ].map(({ Icon, label, color, bg }) => (
                  <div key={label} className="flex flex-col items-center gap-2 py-3 rounded-xl bg-white/3 border border-white/5">
              ...
              <p className="text-center text-xs text-slate-600 mb-4">Sign in with your Google account</p>

              {/* Google button */}
              <button
                onClick={handleGoogleSignIn}
                disabled={isLoading}
                className="w-full flex items-center justify-center gap-3 bg-white hover:bg-slate-50 text-slate-800 rounded-xl px-5 py-3.5 text-sm font-semibold transition-all duration-150 shadow-lg hover:shadow-xl disabled:opacity-60 disabled:cursor-not-allowed active:scale-[0.98]"
              >
                {isLoading ? (
